...
class DownloadService:
    ...
    def _clean_filename(filename: str) -> str:
        """Clean up filename by removing trailing 'filename_...' and extra spaces."""
        # Remove anything after '._; filename_' or similar patterns
        import re
        # Remove the pattern: .mp4_; filename_=UTF-8''...
        cleaned = re.split(r'\.(mp4|m4a|mp3|webm|mov|avi|flv|wmv|mkv)_; filename_=', filename, flags=re.IGNORECASE)[0]
        # Remove trailing spaces and dots
        cleaned = cleaned.rstrip(' ._')
        # If extension was removed, add it back
        m = re.search(r'\.(mp4|m4a|mp3|webm|mov|avi|flv|wmv|mkv)', filename, re.IGNORECASE)
        if m and not cleaned.lower().endswith(m.group(0).lower()):
            cleaned += m.group(0)
    ...
    def download_media(url: str, download_type: str, format_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        import tempfile
        temp_dir = os.path.join(tempfile.gettempdir(), "media_downloads")
                    ...
                    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                        ...
                        if info:
                            ...
                            if os.path.exists(actual_filename):
                                # Clean up filename for YouTube/TikTok style
                                cleaned_filename = DownloadService._clean_filename(os.path.basename(actual_filename))
                                cleaned_filepath = os.path.join(os.path.dirname(actual_filename), cleaned_filename)
                                if cleaned_filepath != actual_filename:
                                    os.rename(actual_filename, cleaned_filepath)
                                    actual_filename = cleaned_filepath
